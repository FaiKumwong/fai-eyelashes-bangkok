          ...
          <div className="md:hidden absolute top-full left-0 w-full bg-gradient-to-r from-pink-500 to-purple-500 py-4">
            <div className="flex flex-col items-center space-y-4 px-6">
              <div className="flex justify-center space-x-4 mb-4">
                <button
                  onClick={() => setLanguage('en')}
                  className={`flex items-center ${language === 'en' ? 'text-white' : 'text-white/70'}`}
                >
                  <svg className="w-6 h-4" viewBox="0 0 60 30">
                    <clipPath id="tm">
                      <path d="M30,15 h30 v15 z v15 h-30 z h-30 v-15 z v-15 h30 z"/>
                    ...
                    <path d="M0,0 v30 h60 v-30 z" fill="#00247d"/>
                    <path d="M0,0 L60,30 M60,0 L0,30" stroke="#fff" strokeWidth="6"/>
                    <path d="M0,0 L60,30 M60,0 L0,30" clipPath="url(#tm)" stroke="#cf142b" strokeWidth="4"/>
                    <path d="M30,0 v30 M0,15 h60" stroke="#fff" strokeWidth="10"/>
                    <path d="M30,0 v30 M0,15 h60" stroke="#cf142b" strokeWidth="6"/>
                  </svg>
                </button>
                <button
                  onClick={() => setLanguage('th')}
                  className={`flex items-center ${language === 'th' ? 'text-white' : 'text-white/70'}`}
                >
                  <svg className="w-6 h-4" viewBox="0 0 900 600">
                    <rect fill="#ED1C24" width="900" height="600"/>
                    <rect fill="#fff" y="100" width="900" height="400"/>
              ...
              <button onClick={() => scrollToSection('services')} className="text-white hover:text-white/80 text-center w-full">
                {translations[language].nav.services}
              </button>
              <Link to="/gallery" className="text-white hover:text-white/80 text-center w-full">
                {translations[language].nav.gallery}
              </Link>
              <button onClick={() => scrollToSection('testimonials')} className="text-white hover:text-white/80 text-center w-full">
                {translations[language].nav.testimonials}
              </button>
              <button onClick={() => scrollToSection('contact')} className="text-white hover:text-white/80 text-center w-full">
                {translations[language].nav.contact}
              </button>
              <Link to="/blog" className="text-white hover:text-white/80 text-center w-full">
                {translations[language].nav.blog}
              </Link>
            </div>
          </div>
